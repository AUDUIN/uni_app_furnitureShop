	...
	<view class="uni-flex uni-column  bgc">
		<view class="flex-item flex-item-V uni-bg-red">
			...
			<view class="book">
				...
				<text class="booklogin ml" @tap="handleMyCenter('sign')">注册</text>
			</view>
		</view>
		<view class="flex-item flex-item-V uni-bg-green second-box1">
			<view class=" second-box ">
				<view class="box">
					<input type="confirmtype" class="e-mail" value="" placeholder="请输入邮箱" @blur="checkmail($event.detail.value)" v-model="email" />
					<image class="human" src="../../static/login/ion-ios-person%20-%20Ionicons.png" mode="aspectFill"></image>
				</view>
				<view class="box2">
					<input type="password" class="pwd" value="" placeholder="请输入密码" @input="search($event)" @blur="checkpwd($event.detail.value)" />
				...
				<view class="box3">
					...
					<button class="btn-login" type="primary">登录</button>
				</view>
			</view>
...
<script>
import Rap from './Rap2.js';
export default {
	data() {
		return {
			email:'185014757@qq.com',
